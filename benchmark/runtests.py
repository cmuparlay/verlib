...
def runtest(test,procs,n,z,mix,ts,params,filename,bindir) :
    ...
    runstring("PARLAY_NUM_THREADS=" + str(num_threads) + " numactl -i all ./" + bindir + test + " " + str_time + str_rounds + str_n + str_mfind + str_mix + str_rs + str_proc + str_zipfians + str_trans_size + str_other, filename)

def run_tests(tests, sizes, params, filename, bindir) :
    for test in tests :
                    ...
                    for ts in params.trans_sizes :
                        for z in params.zipfians :
                            for p in params.processors :
                                runtest(test + suffix, p, n, z, mix, ts, params, filename, bindir)
                with open(filename, "a") as f:
                    f.write("...\n")


def run_all(params, filename, bindir="") :
    if test_only :
        params.time = .2
        params.zipfians = [.99]
