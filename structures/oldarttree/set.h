...
struct Set {
  ...
  bool add_child(node* gp, node* p, const K& k, const V& v) {
    if (p->nt == Indirect && !is_full(p)) {
	  ...
	  if (child_ptr == nullptr && gp->nt == 2) {
	    auto gps = (sparse_node*) gp;
	    for (int i=0; i < gps->num_used; i++) 
	      std::cout << gps->keys[i] << ", " ;
		...
		*child_ptr = (node*) full_pool.new_init([=] (full_node* f_n) {
		...
		if (is_full(p)) {
		  ...
		  *child_ptr = (node*) indirect_pool.new_init([=] (indirect_node* i_n) {
		  ...
		  //std::cout << "sparse to sparse: " << s_n->byte_num << ", " << s_n->num_used << std::endl;
		  //print(gp);
  ...
  bool insert(node* root, const K& k, const V& v) {
	...
	if (cptr != nullptr) {// child pointer exists, always true for full node
		...
		// leaf nodes (one of them the original leaf)
		  ...
		  //std::cout << "sparse new pair" << std::endl;
